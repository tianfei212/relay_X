...
func handleConnection(conn net.Conn) {
	...
	if client.Role == RoleFE {
		...
		snap := snapshotPortState()
		_ = client.Send(BaseMessage{Type: MsgPortState, Payload: snap})
			...
			if err := json.Unmarshal(nextRawMsg.Payload, &portPayload); err == nil {
				if client.Role == RoleBE {
					...
					for _, p := range zmqPorts {
						if err := relayv2.GlobalRelay.StartRelayPortWithProtocol(p, "zmq"); err != nil {
							common.LogError("启动中转端口失败", "protocol", "zmq", "port", p, "error", err)
						}
					}
					for _, p := range srtPorts {
						if err := relayv2.GlobalRelay.StartRelayPortWithProtocol(p, "srt"); err != nil {
...
func startTelemetry(c *Client) {
	...
	for range ticker.C {
		...
		for _, p := range ports {
			if sess := relayv2.GlobalRelay.GetSession(p); sess != nil {
