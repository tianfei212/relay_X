...
type RelayServer struct {
	...
	sessions      map[int]*PipeSession // port -> session
	busy          map[int]bool         // port -> busy
...
func (s *RelayServer) acceptLoop(port int, l net.Listener) {
	for {
		c, err := l.Accept()
		if err != nil {
		...
		common.LogInfo("端口收到连接", "port", port, "addr", c.RemoteAddr(), "role", role, "client_id", id)
...
func (s *RelayServer) wrapConnAndHello(c net.Conn) (*BufferedConn, *DataHello) {
